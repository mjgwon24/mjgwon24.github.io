...
const StackCard: React.FC<StackCardProps> = ({ title, items }) => {
    return (
        <div className="flex flex-col gap-4 w-60 sm:w-72 rounded-lg pt-3 pb-5 px-4 bg-black-05p">
            ...
            <div className="flex flex-row gap-2 flex-wrap text-sm weight-400 sm:weight-500">
                {items.map((item, index) => (
                    <div key={index} className="flex flex-col gap-2 rounded-md px-3 py-1.5 bg-black-05p">
                        <span>{item}</span>
                    </div>
                ))}
            </div>
