...
고객의 소비 패턴을 자체 AI가 분석하여, 건전한 소비는 금리 인하로, 과소비는 금리 상승으로 즉각 반영하는 동적 금리 시스템 도입으로, 청년층은 자신의 소비 습관에 따라 직접 금리를 관리할 수 있으며, 기존의 전통적인 신용평가와 다른 새로운 금융 경험을 할 수 있습니다.

우리FIS에서 제공받은 50만 건의 실제 카드 소비 데이터를 바탕으로 머신러닝 기반 금리 예측 모델을 직접 구축했으며, NICE 신용평가 방식과 소비 패턴을 결합해 FLEXRATE만의 신용 점수를 산출합니다. 데이터 전처리, 파생 변수 생성, 연령대와 소비 패턴별 계수 적용 등 실제 정책을 수식화하여 개인별 맞춤 금리를 제공합니다.

또한, 게이미피케이션 요소를 적용해 사용자가 자신의 소비 목표를 설정하고 달성할 때마다 금리가 일 0.01%포인트씩 자동 인하되는 우대금리 시스템을 설계했습니다.
...
- 관리자/사용자 권한 분리 구조 설계
### 개발
- 실제 카드 소비 데이터 50만 건을 사용해 결측값 보정과 파생 변수 생성 **데이터 전처리** 수행
...
- \`Github Action\`, \`Shell Script\`를 활용한 **자동 배포 환경** 구축
...
### 운영
- 로드밸런싱을 통한 **트래픽 분산 및 장애 대응 체계** 구축 
...
- MySQL 데이터베이스 **주기적 스냅샷 및 백업 정책** 수립 및 운영
        ...
        description: '2024 경북 해커톤, 동국대학교 축제에서 누적 1,300명 이상의 사용자들에게 특별한 추억을 만들어준 사진 촬영 서비스입니다.',
...
`
            },
            {
                ...
                contributions: `
### 개발  
- \`React\`와 \`Tailwind\`를 사용한 반응형 UI 및 SPA 개발  
- 공통 UI 컴포넌트 설계 및 재사용
