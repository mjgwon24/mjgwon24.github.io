...
### 주요 개선 방안
- **동기 → 비동기 구조 전환** (\`@Async\`, Executor 활용)
- 이미지 업로드 시 실시간 최적화 및 불필요한 임시 파일 제거
- DTO 자료형 개선(불필요한 메모리 사용 최소화)
- 프레임 이미지 캐싱 및 병합 프로세스 단순화
- \`@Scheduled\` 기반 자동 삭제 로직 구현
- API별 응답/에러/처리량 실시간 로깅
...
현재 스택네컷 프로젝트에서는 작은 단위의 기능에만 \`synchronized\` 락을 적용하여 큰 병목 현상이 발생하지 않았습니다. 게다가 아직까지 락 경쟁이 발생할 정도로 많은 요청이 들어온 적이 없어 성능 저하도 발생하지 않았습니다. 앞으로 확장성을 고려하여 필요시 더 세밀한 동기화 방법을 검토할 계획입니다.
            ...
            problemSolving: {
                ...
                content: `
# 비동기 방식 도입으로 인한 동시 디렉토리 생성 문제
\`CompletableFuture.supplyAsync\`를 사용하여 여러 스레드가 동시에 동일한 디렉토리에 접근하는 상황이 있었습니다. 디렉토리가 존재하지 않을 경우, 각 스레드가 디렉토리를 생성하려고 시도하게 됩니다. 이 과정에서 두 개 이상의 스레드가 동시에 \`groupDirectory.exists()\`를 호출한 후, 디렉토리 생성에 들어갈 가능성이 있습니다. 이러한 경우, "디렉토리 생성 실패"나 "IOException" 오류가 발생할 수 있습니다.
<br/><br/>
...
이러한 방식으로 디렉토리 생성 작업이 원자적으로 수행되어 다른 스레드가 동시에 같은 디렉토리를 생성하려는 시도를 방지할 수 있었습니다.
            ...
            retrospective: {
                ...
                content: `
비동기 아키텍처와 이미지 처리 최적화로 최대 **66%의 응답 시간 개선**과 **40%의 데이터 전송량 감소**를 달성하여, 고부하 환경에서도 안정성을 확보했습니다.
<br /><br />
초기 소규모 사용자 환경에서는 동기 방식이 적합했지만, 사용자 수 증가에 따라 일부 로직의 비동기 처리 필요성을 느끼게 되었습니다. 이번 경험을 통해 **상황에 맞는 기술 선택의 중요성**을 다시한번 느끼게 되었습니다.
<br /><br />
JMeter를 활용해 최적화 방법을 직접 검증하고 성능 개선 결과를 확인할 때 매우 즐겁고 뿌듯했습니다. 이번 프로젝트는 함께한 팀원들의 첫 프로젝트였던 만큼, 모든 팀원이 많은 것을 배우고 지식을 얻어갈 수 있었던 보람찬 경험이었습니다.
                `
            }
        }
    },
